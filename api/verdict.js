// Vercel 서버리스 함수: 브라우저 대신 이 함수가 Anthropic API를 호출한다.
// ANTHROPIC_API_KEY는 여기(서버) 환경변수에만 존재하며 클라이언트에 노출되지 않는다.
...
module.exports = async (req, res) => {
  ...
  if (!apiKey) {
    res.status(500).json({ error: "server_misconfigured", message: "서버에 ANTHROPIC_API_KEY가 설정되지 않았어요." });
    return;
  }

  const { itemName, price, category, reason, model } = req.body || {};

  if (typeof itemName !== "string" || itemName.trim() === "" || !Number.isFinite(price) || price <= 0) {
    res.status(400).json({ error: "invalid_input", message: "품목명과 가격을 확인해주세요." });
    return;
  }
  ...
  try {
    ...
    res.status(504).json({ error: code, message: "Anthropic 서버 응답이 지연되고 있어요." });
    return;
  }
  clearTimeout(timeoutId);
